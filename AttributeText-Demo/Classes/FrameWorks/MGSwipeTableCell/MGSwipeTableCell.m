...
-(void) dealloc {
	for (UIView * button in _buttons) {
		if ([button isKindOfClass:[UIButton class]]) {
			[(UIButton *)button removeTarget:self
									  action:@selector(mgButtonClicked:)
							forControlEvents:UIControlEventTouchUpInside];
		}
	}
...
-(void) fixRegionAndAccesoryViews {
	...
	if (self.bounds.size.width != self.contentView.bounds.size.width && [self isRTLLocale]) {
		_swipeOverlay.frame = CGRectMake(-self.bounds.size.width + self.contentView.bounds.size.width,
										 0, _swipeOverlay.bounds.size.width,
										 _swipeOverlay.bounds.size.height);
	}
}
...
-(void)fetchButtonsIfNeeded {
	if (_leftButtons.count == 0 && _delegate
		&& [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
		_leftButtons = [_delegate swipeTableCell:self
						swipeButtonsForDirection:MGSwipeDirectionLeftToRight
								   swipeSettings:_leftSwipeSettings
							   expansionSettings:_leftExpansion];
	}
	if (_rightButtons.count == 0 && _delegate
		&& [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
		_rightButtons = [_delegate swipeTableCell:self
						 swipeButtonsForDirection:MGSwipeDirectionRightToLeft
									swipeSettings:_rightSwipeSettings
								expansionSettings:_rightExpansion];
	}
	
	NSLog(@"left: %@", _leftButtons);
	NSLog(@"right: %@", _rightButtons);
}
/// 创建 swipe界面
-(void)createSwipeViewIfNeeded {
	...
	/// 遮罩界面
	if (!_swipeOverlay) {
		_swipeOverlay = [[UIView alloc] initWithFrame:CGRectMake(0, 0,
	...
	if (!_leftView && _leftButtons.count > 0) {
		...
		_leftView.cell = self;
		_leftView.frame = CGRectMake(-_leftView.bounds.size.width,
	...
	if (!_rightView && _rightButtons.count > 0) {
		...
		_rightView.frame = CGRectMake(_swipeOverlay.bounds.size.width,
...
-(UIColor *) backgroundColorForSwipe {
	...
	else if (self.contentView.backgroundColor
...
-(void)hideSwipeAnimated: (BOOL) animated completion:(void(^)(BOOL finished)) completion {
	MGSwipeAnimation * animation = animated ? (_swipeOffset > 0
											   ? _leftSwipeSettings.hideAnimation
											   : _rightSwipeSettings.hideAnimation) : nil;
...
-(void) setSwipeOffset:(CGFloat)offset
			 animation: (MGSwipeAnimation *) animation
